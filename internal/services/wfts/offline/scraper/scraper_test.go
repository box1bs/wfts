...
func TestHtmlGetter(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            html, err := NewScraper(&configData{}, nil, nil, context.Background()).getHTML(context.Background(), tt.url, NewRateLimiter(1), 3)
            if err != nil {
...
func TestHaveSitemap(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            links, err := NewScraper(&configData{}, nil, nil, context.Background()).haveSitemap(parsed)
            if err != nil {
