...
func (idx *indexer) HandleDocumentWords(ctx context.Context, doc *model.Document, priority *float64, passages []model.Passage) error {
	...
	*priority += math.Log(float64(len(pos)) + 1) // (1 + sameDomain) * (log(linksNumber + 1) + log(UniqTokenCount + 1)) / ((parentDepth + 1) * (log(tokenCount + 1) + 1)) * e**(-a * scrapedDepth - depth) // наивная метрика приоритизации
