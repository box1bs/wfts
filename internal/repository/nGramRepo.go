...
func NewWordIndex(bufSize uint32) *ngChunkIndex {
...
func (ir *IndexRepository) IndexTriGrams(words []string) error {
	for _, word := range words {
		...
		req := srequest{word: word, seqC: make(chan *sequence, 1)}
		var seq *sequence
		shardKey := word[0]
		...
		for _, ng := range extractTGrams(word) {
			...
			if err := binary.Write(ir.ni.chunks[bkey], binary.LittleEndian, encNGData(seq.id, ng[2])); err != nil {
				ir.ni.locks[bkey].Unlock()
				return err
			}
			ir.ni.locks[bkey].Unlock()
